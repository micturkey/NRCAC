...
def generate_data(seed, nodes, max_degree, normal_samples, anomalous_samples, anomalous_nodes, states, verbose=VERBOSE):
    ...
    with open(src_dir +'root.txt', 'w') as file:
        data_to_write = ','.join(an_nodes)
        file.write(data_to_write)

    # Create a random DAG
    bn, g_graph = get_random_dag(nodes, max_degree=max_degree, states=states)
    # save_edges_to_file(g_graph, src_dir + 'edges.txt')
    edge_list = []
    for edge in g_graph.get_graph_edges():
        edge_list.append([edge.node1.name, edge.node2.name])
    # if verbose: print(edge_list)
    # 输出结果（或保存到文件）
    with open(src_dir + 'dependency_list.txt', 'w') as file:
        file.write(str(edge_list))
    # print(g_graph.get_edge)
...
# def save_edges_to_file(graph, filename):
