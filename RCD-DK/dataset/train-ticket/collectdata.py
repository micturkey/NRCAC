...
NODE_IP_MAPPING = {
    "k8s-2": "192.168.3.208:10250",
    "k8s-3": "192.168.3.84:10250",
    "k8s-4": "192.168.3.109:10250"
}
NODE_QUERIES = {
    "node_cpu_k8s": f'sum(rate(node_cpu_seconds_total{{instance=~"k8s-[2-4]", job="node-exporter", mode!="idle"}}[{DURATION}])) by (instance) * 100',
    "node_cpu_ip": f'sum(rate(container_cpu_usage_seconds_total{{namespace="{TRAIN_TICKET_NS}"}}[{DURATION}])) by (instance) * 100',
...
if __name__ == '__main__':
    ...
    # Calculate node_cpu and store it in a new column
    for node in NODES:
        k8s_key = f"{node}_node_cpu_k8s"
        ip_key = f"{NODE_IP_MAPPING[node]}_node_cpu_ip"
        if k8s_key in node_data and ip_key in node_data:
            k8s_cpu_values = np.array(node_data[k8s_key]).astype(float)
            ip_cpu_values = np.array(node_data[ip_key]).astype(float)
            if len(k8s_cpu_values) == len(ip_cpu_values):
                node_data[f"{node}_node_cpu"] = k8s_cpu_values - ip_cpu_values
            else:
                print(f"Data length mismatch for {node}: k8s data length is {len(k8s_cpu_values)}, ip data length is {len(ip_cpu_values)}")

    # Remove the intermediate columns
    columns_to_remove = [f"{node}_node_cpu_k8s" for node in NODES] + [f"{ip}_node_cpu_ip" for ip in NODE_IP_MAPPING.values()]
