...
def run_pc(data, alpha, localized=False, labels={}, mi=[], verbose=VERBOSE, knowledge = None):
    if labels == {}:
    ...
    if localized:
        ...
        cg = SkeletonDiscovery.local_skeleton_discovery(np_data, f_node, alpha,
                                                        ...
                                                        labels=labels, verbose=verbose, background_knowledge = knowledge)
    else:
...
def top_k_rc(normal_df, anomalous_df, bins=None, mi=[],
             ...
             verbose=VERBOSE, knowledge = None):
    ...
    # print("min_nodes", min_nodes, "data", len(data))
    ...
    if verbose: print("i to label", i_to_labels)
    ...
    # knowledge_i = [[labels_to_i[node] for node in pair if node in labels_to_i] for pair in knowledge]
    knowledge_i = None
    if knowledge is not None:
        knowledge_i = [[labels_to_i[node] for node in pair] for pair in knowledge if all(node in labels_to_i for node in pair)]
        if verbose: print("nl_i",knowledge_i)

    _run_pc = lambda alpha: run_pc(data, alpha, localized=localized, mi=processed_mi,
                                   labels=i_to_labels, verbose=verbose, knowledge = knowledge_i)
